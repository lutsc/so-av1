...
O código-fonte está disponível publicamente em repositório no Github em \url{https://github.com/lutsc/so-av2/}.
...
\begin{enumerate}
  ...
  \item \texttt{img\_worker} cria $N$ \textit{threads} e enfileira $Q$ \textit{tasks} baseado em um limite arbitrário;
  \item Cada \textit{thread} consome uma \textit{task} e processa as linhas correspondentes em \texttt{g\_out};
  \item Cada \textit{thread} é encerrada quando ela não possuí mais \textit{tasks} para receber;
...
// Abre a FIFO para leitura em binário, bloqueando até que o processo emissor a abra para escrita
...
// Lê os metadados e os pixels da FIFO
Header header;
...
// Prepara a estrutura de dados de saída com os mesmos metadados da imagem de entrada
g_out.w = g_in.w;
g_out.h = g_in.h;
...
// Calcula os blocos de linhas para cada tarefa da fila estática
int32_t Q_Rows;
...
// Espera todas as threads terminarem o processamento antes de escrever a imagem de saída
for(int i = 0; i < g_nthreads; i++){
  pthread_join(threads[i], NULL);
}
write_pgm(args.output_file, &g_out);
  
// Destroí semáforo e libera recursos alocados
sem_destroy(&sem);
free(g_in.data);
free(g_out.data);
fflush(fd);
...
void* worker_thread(void* arg){
  while(1){
    // Aguarda o semáforo para acessar a fila de tarefas
    sem_wait(&sem);
    Task task = queue_buf[q_count];
    q_count++;
    sem_post(&sem);

    // Processa o bloco de linhas correspondente à tarefa
    ...
    // Encerra a thread se todas as tarefas foram consumidas
    if(q_count >= QMAX)
    {
      pthread_exit(NULL); 
    }
